...
function AddMemberForm({ groupId, people, onDone }: { groupId: string; people: PersonWithBalance[]; onDone: () => void }) {
  ...
  return <form action={submit} className="mt-4 flex flex-wrap items-end gap-3 border-t border-slate-800 pt-3"><input type="hidden" name="groupId" value={groupId} /><label className="grid flex-1 gap-1.5 text-sm text-slate-300">Friend<select name="personId" required defaultValue="" className={INPUT}><option value="" disabled>Choose a friend</option>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><button type="submit" disabled={pending || people.length === 0} className={`${BUTTON} bg-emerald-500 text-slate-950 hover:bg-emerald-400`}>{pending ? "Adding…" : "Add member"}</button>{people.length === 0 && <p className="w-full text-sm text-amber-300">All active friends are already members.</p>}{state.error && <ErrorNote message={state.error} />}</form>;
}
